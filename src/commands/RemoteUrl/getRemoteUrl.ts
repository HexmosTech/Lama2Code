    ...
    try {
      ...
      if (!remoteURL.startsWith("git@") && !remoteURL.startsWith("https")) {
        vscode.window.showErrorMessage(
          "URL not found. Currently supports Github only (both HTTPS and SSH)"
        );
      } else {
        if (remoteURL.startsWith("git@")) {
          ...
          const stripdotgit =
            remoteURL.substring(0, remoteURL.length - 4) + "/blob/"; // remove .git
          gitURLPrefix = "";
        }
        const stripdotgit =
          gitRepoURL.substring(0, gitRepoURL.length - 4) + "/blob/"; // remove .git
        const replacecolon = stripdotgit.replace(":", "/"); // replace colon with forward slash
        const result =
          gitURLPrefix + replacecolon + currentBranch + "/" + relativePath;
        vscode.window.showInformationMessage(
          "Remote URL successfully found and copied to clipboard"
        );
