...
import { ErrorCodes, IJSONRPCResponse, logToChannel } from "../response/generalResponse"
...
function isCursorInsideStringVarContainer(lineText: string, cursorCharacter: number): boolean {
...
export function lama2ProvideCompletionItems(langServer: ChildProcess, requestId: number) {
  return {
    async provideCompletionItems(
      ...
      const searchQuery = currentLine.substring(openingBraceIndex + 2, cursorIndex)

      const response: IJSONRPCResponse = await getEnvsFromLsp(langServer, requestId, document, position, searchQuery)
      logToChannel({
