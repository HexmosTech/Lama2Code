      ...
      if (specificEx) {
        ...
        if (specificEx.endsWith(".env") || specificEx.endsWith(".l2")) {
          vscode.window.activeTextEditor?.edit(builder => {
            ...
            builder.replace(new vscode.Range(doc!.lineAt(0).range.start, doc!.lineAt(doc!.lineCount - 1).range.end), examplesJSON[specificExGlobal]);
          });
          return
        }
        ...
        if (subSpecificEx) {
          ...
          vscode.window.activeTextEditor?.edit(builder => {
            const doc = vscode.window.activeTextEditor?.document;
            builder.replace(new vscode.Range(doc!.lineAt(0).range.start, doc!.lineAt(doc!.lineCount - 1).range.end), examplesJSON[specificExGlobal][subSpecificExGlobal]);
          });
          return
