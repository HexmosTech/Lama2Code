...
J2HConverter.prototype = {
  ...
  _getJsonStructure: function (parsedJson) {
    if (parsedJson && parsedJson.length > 0) {
      if (typeof parsedJson[0] == "string") return this._jsonStruct.stringArray;
        ...
        return this._jsonStruct.objectArray;
    }
  ...
  _setAttributes: function (srcTable, attributes) {
    if (attributes) {
      for (var key in attributes) srcTable.setAttribute(key, attributes[key]);
    } else {
      srcTable.border = "1";
      ...
      colData.forEach(function (colDataItem, colDataItemIdx) {
        ...
        if (jsonStruct == this._jsonStruct.stringArray)
          ...
          if (rowDataItem.hasOwnProperty(colDataItem)) {
            var colDataStr = this._jsonObjToArray(
              JSON.stringify(rowDataItem[colDataItem])
            );
            var colDataJson = JSON.parse(colDataStr);
            var colDataStruct = this._getJsonStructure(colDataJson);
            if (
              colDataStruct == this._jsonStruct.unknown ||
              colDataStruct == this._jsonStruct.stringArray
            ) {
  ...
  _toggleDetail: function (e) {
    ...
    if (e.target.args.colState == "collapsed") {
      if (e.target.args.processRowData) {
        ...
        // Just display on subsequent toggles
        parentRowIdx = e.target.args.parentRowIdx;
        trHeading = srcTable.tBodies[0].querySelector(
          "#trh_" + parentRowIdx + "_" + srcColumn
        );
        trDetail = srcTable.tBodies[0].querySelector(
          "#trd_" + parentRowIdx + "_" + srcColumn
        );
